...
SCRIPT_FAILURE_MSG = 'Seems like smth is broken. Please copy an error below' \
                     ' and report it to me by using GitHub, https://vk.com/zema or https://t.me/damnware. Thanks!'
MAX_INPUT_ATTEMPTS = 3  # Define your own number of attempts
...
def scan_script(beautified_lines: List[str], scan_type: str) -> Optional[List[str]]:
    ...
    for line in beautified_lines:
        ...
        if "2" in scan_type:
            regex_find_result = re.findall(
                r"(http|ftp|https)://([\w_-]+(?:\.[\w_-]+)+)([\w.,@?^=%&:/~+#-]*[\w@?^=%&/~+#-])", line)
            if regex_find_result and not any(url.lower() in line.lower() for url in IGNORE_URL_LIST):
                regex_search_result = re.search("(?P<url>https?://\S+)", line).group("url")
                print(f"Found link: {regex_search_result}")
        if "3" in scan_type:
            regex_find_result = re.findall(r"[a-zA-Z\d]{25,100}", line)
            if regex_find_result and not any(word.lower() in line.lower() for word in IGNORE_WORD_LIST):
