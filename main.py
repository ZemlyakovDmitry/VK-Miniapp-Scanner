...
ignore_list = ["yandexMap", "VKWebApp", "reactInternal", "component", "Icon", "Browser", "Scroll", "get", "Location",
               "Panel", "Point", "Hook", "animat", "remove", "event", "mouse", "refresh", "padding", "child"]
...
def scanner(scripts_url, scan_type):
    for file in scripts_url:
        ...
        if "2" in scan_type:
            for line in lines:
                if 'w3.org' not in line and 'reactjs.org' not in line and re.findall(
        ...
        if "3" in scan_type:
            for line in lines:
                if re.findall(r"[a-zA-Z\d]{25,100}", line) and not any(
                        word.lower() in line.lower() for word in ignore_list):
